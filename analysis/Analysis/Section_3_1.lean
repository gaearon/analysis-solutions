...
theorem SetTheory.Set.eq_empty_iff_forall_notMem {X:Set} : X = ∅ ↔ (∀ x, x ∉ X) := by
  constructor
  · intro h x
    simp only [h, not_mem_empty, not_false_eq_true]
  intro h
  simp only [h, ext_iff, not_mem_empty, implies_true]
...
theorem SetTheory.Set.singleton_union_singleton (a b:Object) :
  ...
  apply ext
  intro x
  simp only [mem_union, mem_pair, mem_singleton]
...
theorem SetTheory.Set.union_empty (A:Set) : A ∪ ∅ = A := by
  ...
  intro x
  rw [mem_union]
  simp only [not_mem_empty, or_false]
